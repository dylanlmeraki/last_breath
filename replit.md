...
**Components (14)**: AnimatedGridBackground, AnimatedSection (parallax support), BackToTop (50% scroll threshold, lower-left), BlueprintBackground, ChatBot (draggable, starts as icon, auto-opens at 50% scroll, won't re-open after manual close), FloatingElements (architectural blueprint SVGs), FooterBackground, GlowingCard, ParticleField, ScrollFadeSection, SectionDivider (angled/gradient/blueprint variants — angled used consistently between homepage sections), SEO, ShinyButton

**Design System**: Cyan = precision engineering brand color; Orange = action/CTA only ("Free Consultation" always orange). Service cards use darkblue→bluecyan→cyan→cyanteal color scheme (blue→teal family, no orange/emerald). Homepage includes mid-page CTA with slanted clip-path edge after services, testimonial section with star rating, and bottom CTA. ChatBot uses gradient MessageCircle icon (cyan→blue). Desktop nav: full-height items with hover bg + bottom border accent, dropdowns with border-t-4 blue, ShinyButton "Free Consultation" CTA. Mobile nav: fullscreen overlay with dropdown arrows for Services/About + orange "Call" pill button next to hamburger (subtitle hidden on xs). Hero: GGB draft video with BlueprintBackground animation overlay (thinner lines, architectural detail elements). Favicon: PE logo (image_1773808313870.png).
