...
export default function ClientInvoices() {
  ...
  const totalPaid = myInvoices.filter((i) => i.status === "paid").reduce((s, i) => s + (i.total_amount || 0), 0);
  const totalOutstanding = myInvoices.filter((i) => i.status === "sent" || i.status === "overdue").reduce((s, i) => s + (i.total_amount || 0), 0);
  ...
  return (
    <div>
      ...
      {isLoading ? <TableSkeleton /> : myInvoices.length === 0 ? (
